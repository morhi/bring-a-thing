...
it('reuses an existing user for a shared invite email across different groups', function () {
    ...
    $this->actingAs($owner)->post(route('groups.invite', $groupOne), ['email' => 'invitee@example.com']);
    $this->actingAs($owner)->post(route('groups.invite', $groupTwo), ['email' => 'invitee@example.com']);
...
it('rejects inviting the owner\'s own email', function () {
    ...
    $response = $this->actingAs($owner)->post(route('groups.invite', $group), ['email' => $owner->email]);

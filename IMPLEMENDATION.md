...
- **Confirmation modal on claim-while-unavailable** (added on request, beyond original phase scope): clicking a claim action while the attendance warning tag applies now opens a `confirm.require()` modal (`RosterItemCard.vue`, reusing the app's existing PrimeVue `ConfirmDialog`/`useConfirm` pattern) instead of submitting immediately, across all four claim entry points that show the warning — self claim (quantity-null item), quantity claim, and both owner/admin "claim for" flows. The passive warning tag is unchanged; the modal is an added confirmation step, not a replacement for it.
- **Always-visible attendance-day link with a jump-to-poll link** (added on request, beyond original phase scope): `RosterItemCard.vue` and the roster header in `Rosters/Show.vue` now always render the resolved attendance day (`item.effective_attendance_poll_option` / `roster.attendance_poll_option`) as a label next to the date tag, formatted with the existing `formatPollOption()` helper, linking to the poll's show page (`PollController@show`) via the existing Wayfinder `showPoll()` helper so the viewer can go change their own vote. Required eager-loading `attendancePollOption.poll` in `RosterController@show` (previously only `attendancePollOption.responses.user` was loaded, so the parent poll wasn't available to link to) and adding `poll?: Poll` to the frontend `PollOption` type.
